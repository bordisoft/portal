...
Class Helper
{
	...
	public static function getToken($regenerate=NULL)
	{
		$token = Cache::getInstance()->getCache('token');
		$token = $token && !$regenerate ? $token : substr(md5(mt_rand()*time()*mt_rand()),0,20);
		Cache::getInstance()->setCache('token',$token);
	...
	public static function GetStatusHtml($status)
	{
		...
		return $icon ? $icon.' '.$status->name : '';
	}

	public static function ProcessFieldsArray(Array $fields,$form_fields,$class='form-control')
	{
		if(empty($fields)){ return array(); }
		$adjusted = array();
		foreach($form_fields as $name => $options)
		{
			if(isset($fields[$name]))
			{
				$fields[$name] = (array)$fields[$name];
				$updatable = 'update' == $fields[$name]['access'] ? ' name="args['.$name.']"' : ' disabled="disabled"';
				if(isset($options['html']) && 'select' == $options['html'] && (is_object($fields[$name]['value']) || is_array($fields[$name]['value'])))
				{
					$select = '';
					if(isset($options['parent']))
					{
						$parent_selected = 0;
						if(isset($fields[$options['parent']]) && is_array($fields[$options['parent']]['value']))
						{
							foreach($fields[$options['parent']]['value'] as $parent)
							{
								if($parent->sel)
								{
									$parent_selected = $parent->value ? $parent->value : 0;
									break;
								}
							}
						}
						elseif(isset($fields[$options['parent']]))
						{
							$parent_selected = $fields[$options['parent']]['value'];
						}
						else
						{
							continue;
						}
						$select .= '<div id="'.$name.'"><select class="form-control'.(empty($options['mandatory']) ? "" : " mand").'"'.$updatable.' disabled="disabled"></select>';
						foreach((array)$fields[$name]['value'] as $key => $_select)
						{
							$sel = ($key === $parent_selected) ? 1 : 0;
							$select .= '<select class="has-parent" data-parent="'.$key.'" style="display:none;">';
							foreach((array)$_select as $_option)
							{
								$select .= '<option'.($_option->sel ? ' selected="selected"' : '').' value="'.
										$_option->value.'">'.$_option->name.'</option>';
							}
							$select .= '</select>';
						}
						$select .= '</div>';
					...
					{
						$has_child = isset($options['child']) ? 1 : 0;
						$select_class = array($class);
						if($has_child)
						{
							$select_class[] = 'has-child';
						}
						if($options['mandatory'])
						{
							$select_class[] = 'mand';
						}
						$select_class = ' class="'.implode(' ',$select_class).'"';
						$select .= '<select'.($has_child ? ' data-child="'.$options['child'].'"' : '').' id="'.$name.'"'.$select_class.$updatable.'>';
						foreach($fields[$name]['value'] as $option)
						{
							$select .= '<option'.(!empty($option->sel) ? ' selected="selected"' : '').' value="'.$option->value.'">'.
									$option->name.'</option>';
						}
						$select .= '</select>';
					}

					$adjusted[$name] = array('html' => $select);
				}
				elseif(isset($options['html']) && 'checkbox' == $options['html'])
				{
					if('update' == $fields[$name]['access'])
					{
						$selection = NULL;
						foreach($fields[$name]['value'] as $v)
						{
							if($v->sel)
							{
								$fields[$name]['value'] = $v->value;
								break;
							}
						}
						$adjusted[$name] = array('html' => '<input '.($options['mandatory'] ? 'class="mand"' : '').'" '.
								'type="checkbox"'.($fields[$name]['value'] == 'Y' ? ' checked="checked"' : '').$updatable.' value="Y" />');
					}
					else
					{
						$adjusted[$name] = array('html' => '<input type="checkbox" disabled="disabled"'.((!empty($fields[$name]['value']) && in_array($fields[$name]['value'],array("Y","Yes"))) ? ' checked="checked"' : '').' />');
					}
				}
				elseif(isset($options['html']) && 'input_hidden' == $options['html'])
				{
					$adjusted[$name] = array('html' => '<input type="hidden" value="'.$fields[$name]['value'].'"'.$updatable.' />');
				}
				else
				{
					$new_class = $class;
					$new_class .= self::_GetFieldClass($options).("email" == $name ? " email" : "");
					if(isset($options['html']) && 'textarea' == $options['html'])
					{
						$adjusted[$name] = array('html' => '<textarea'.$updatable.' class="'.$new_class.'">'.$fields[$name]['value'].'</textarea>');
					}
					else
					{
						$adjusted[$name] = array('html' => '<input type="'.('password' == isset($options['html']) && 'password' == $options['html'] ? 'password' : 'text').'" '.
								'class="'.$new_class.'"'.$updatable.' value="'.$fields[$name]['value'].'" />');
					}
				}

				if(!empty($fields[$name]['title']))
				{
					$adjusted[$name]['title'] = $fields[$name]['title'];
				}
			}
		}
		...
		{
			$value = (array)$value;
			if(!isset($adjusted[$name]))
			{
				$adjusted[$name] = array(
					'html' => '<input type="text" class="'.$class.'" disabled="disabled" value="'.$value['value'].'" />',
					'title' => $value['title']
				);
			}
		}
	...
	public static function DateTimeFormatCovertToPHP($string) {
		//time
		$string = preg_replace('/HH24/', 'H', $string);
		$string = preg_replace('/HH12/', 'h', $string);
		$string = preg_replace('/HH/', 'H', $string);
		$string = preg_replace('/MI/', 'i', $string);
		$string = preg_replace('/AM/', 'A', $string);
		$string = preg_replace('/SS/', 's', $string);
		//date
		$string = preg_replace('/YYYY/', 'Y', $string);
		$string = preg_replace('/YY/', 'y', $string);
		$string = preg_replace('/DD/', 'd', $string);
		$string = preg_replace('/D/', 'j', $string);
		$string = preg_replace('/MONTH/', 'F', $string);
		$string = preg_replace('/MON/', 'M', $string);
		$string = preg_replace('/MM/', 'm', $string);
		$string = preg_replace('/M/', 'n', $string);
		return $string;
	}

	public static function DateTimeFormatCovertToJS($string) {
		//time
		$string = preg_replace('/HH24/', 'hh', $string);
		$string = preg_replace('/HH12/', 'h', $string);
		$string = preg_replace('/HH/', 'hh', $string);
		$string = preg_replace('/MI/', 'mm', $string);
		$string = preg_replace('/AM/', 'A', $string);
		$string = preg_replace('/SS/', 'ss', $string);
		//date
		$string = preg_replace('/YYYY/', 'yyyy', $string);
		$string = preg_replace('/YY/', 'yy', $string);
		$string = preg_replace('/DD/', 'dd', $string);
		$string = preg_replace('/D/', 'd', $string);
		$string = preg_replace('/MONTH/', 'mm', $string);
		$string = preg_replace('/MON/', 'm', $string);
		$string = preg_replace('/MM/', 'mm', $string);
		$string = preg_replace('/M/', 'm', $string);
		return $string;
	}

// 	public static function GetFields($form,$realm=NULL)
// 	{
// 		static $fields;
// 		if(is_null($fields))
// 		{
...
// 				'info' => array (
// 					'general' => array(
// 							'companyname' => array('mandatory' => 0,'type' => 'varchar'),
// 							'salutation' => array('mandatory' => 0,'type' => 'varchar'),
// 							'firstname' => array('mandatory' => 0,'type' => 'varchar'),
// 							'midinit' => array('mandatory' => 0,'type' => 'varchar'),
// 							'lastname' => array('mandatory' => 0,'type' => 'varchar'),
// 							'state' => array('mandatory' => 0,'type' => 'varchar'),
// 							'zip' => array('mandatory' => 0,'type' => 'varchar'),
// 							'city' => array('mandatory' => 0,'type' => 'varchar'),
// 							'country' => array('mandatory' => 0,'type' => 'varchar'),
// 							'cont1' => array('mandatory' => 0,'type' => 'varchar'),
// 							'phone1' => array('mandatory' => 0,'type' => 'varchar'),
// 							'faxnum' => array('mandatory' => 0,'type' => 'varchar'),
// 							'phone2' => array('mandatory' => 0,'type' => 'varchar'),
// 							'cont2' => array('mandatory' => 0,'type' => 'varchar'),
// 							'email' => array('mandatory' => 0,'type' => 'email'),
// 							'i_vd_plan' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'out_date_format' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'out_time_format' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'out_date_time_format' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'in_date_format' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'in_time_format' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'i_time_zone' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'i_lang' => array('mandatory' => 0,'type' => 'string','html' => 'select'),
// 							'i_distributor' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'login' => array('mandatory' => 0,'type' => 'varchar'),
// 					),
// 					'Customer' => array(
// 							'address' => array('mandatory' => 0,'type' => 'text'),
// 							'discount_rate' => array('mandatory' => 0,'type' => 'double'),
// 							'tax_id' => array('mandatory' => 0,'type' => 'integer'),
// 							'i_billing_period' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'send_invoices' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'send_statistics' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 							'discount_rate' => array('mandatory' => 0,'type' => 'double'),
// 							'tax_id' => array('mandatory' => 0,'type' => 'integer'),
// 							'i_do_batch' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'i_rep' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'override_tarrifs_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 							'i_template' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'i_number_scope' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'shifted_billing_date' => array('mandatory' => 0,'type' => 'varchar'),
// 							'ppm_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 							'drm_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 					),
// 					'Account' => array(
// 							'baddr1' => array('mandatory' => 0,'type' => 'text'),
// 							'ecommerce_enabled' => array('mandatory' => 0,'type' => 'integer','html' => 'checkbox'),
// 							'billing_model' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'i_customer_site' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 							'activation_date' => array('mandatory' => 0,'type' => 'varchar'),
// 							'expiration_date' => array('mandatory' => 0,'type' => 'varchar'),
// 							'life_time' => array('mandatory' => 0,'type' => 'varchar'),
// 					),
// 				),
// 				'make_payment' => array(
// 						'amount' => array('mandatory' => 1,'type' => 'double')
// 				),
// 				'login' => array(
// 						'pb_auth_user' => array('mandatory' => 1,'type' => 'varchar'),
// 						'pb_auth_password' => array('mandatory' => 1,'type' => 'password'),
// 						'remember_me' => array('mandatory' => 0,'type' => 'integer')
// 				),
// 				'paypal' => array(
// 						'amount' => array('mandatory' => 1,'type' => 'double'),
// 						'return_page' => array('mandatory' => 1,'type' => 'varchar','html' => 'hidden'),
// 						'cancel_page' => array('mandatory' => 1,'type' => 'varchar','html' => 'hidden')
// 				),
// 				'payment_info' => array (
// 						'general' => array(
// 							'cc_i_payment_method' => array('mandatory' => 1,'type' => 'integer','html' => 'select'),
// 							'cc_number' => array('mandatory' => 2,'type' => 'varchar'),
// 							'cc_exp_month' => array('mandatory' => 2,'type' => 'integer','html' => 'select'),
// 							'cc_exp_year' => array('mandatory' => 2,'type' => 'integer','html' => 'select'),
// 							'cc_cvv' => array('mandatory' => 2,'type' => 'password'),
// 							'cc_name' => array('mandatory' => 2,'type' => 'varchar'),
// 							'cc_address' => array('mandatory' => 2,'type' => 'varchar'),
// 							'cc_city' => array('mandatory' => 0,'type' => 'varchar'),
// 							'cc_iso_3166_1_a2' => array('mandatory' => 0,'type' => 'varchar','html' => 'select','child'=>'cc_i_country_subdivision'),
// 							'cc_i_country_subdivision' => array('mandatory' => 0,'type' => 'integer','html' => 'select','parent'=>'cc_iso_3166_1_a2'),
// 							'cc_zip' => array('mandatory' => 2,'type' => 'varchar')
// 						),
// 						'Customer' => array(),
// 						'Account' => array()
// 				),
// 				'another_cc_payment' => array (
// 					'another_cc_amount' => array('mandatory' => 1,'type' => 'float'),
// 					'another_cc_i_payment_method' => array('mandatory' => 1,'type' => 'integer','html' => 'select'),
// 					'another_cc_number' => array('mandatory' => 1,'type' => 'varchar'),
// 					'another_cc_exp_month' => array('mandatory' => 2,'type' => 'integer','html' => 'select'),
// 					'another_cc_exp_year' => array('mandatory' => 2,'type' => 'integer','html' => 'select'),
// 					'another_cc_cvv' => array('mandatory' => 1,'type' => 'password'),
// 					'another_cc_name' => array('mandatory' => 1,'type' => 'varchar'),
// 					'another_cc_address' => array('mandatory' => 1,'type' => 'varchar'),
// 					'another_cc_city' => array('mandatory' => 0,'type' => 'varchar'),
// 					'another_cc_iso_3166_1_a2' => array('mandatory' => 0,'type' => 'varchar','html' => 'select','child'=>'another_cc_i_country_subdivision'),
// 					'another_cc_i_country_subdivision' => array('mandatory' => 0,'type' => 'integer','html' => 'select','parent'=>'another_cc_iso_3166_1_a2'),
// 					'another_cc_zip' => array('mandatory' => 1,'type' => 'varchar'),
// 					'alternate' => array('mandatory' => 1,'type' => 'integer','html' => 'hidden')
// 				),
// 				'voucher_topup' => array(
// 					'voucher' => array('mandatory' => 1,'type' => 'varchar')
// 				),
// 				'ch_pass' => array(
// 					'old_password' => array('mandatory' => 1,'type' => 'password'),
// 					'new_password' => array('mandatory' => 1,'type' => 'password'),
// 					'new_password_retype' => array('mandatory' => 1,'type' => 'password')
// 				),
// 				'ppayment'=> array(
// 					'amount' => array('mandatory' => 1,'type' => 'float'),
// 					'i_periodical_payment_period' => array('mandatory' => 1,'type' => 'integer'),
// 					'balance_threshold' => array('mandatory' => 2,'type' => 'float'),
// 					'from_date' => array('mandatory' => 1,'type' => 'varchar'),
// 					'to_date' => array('mandatory' => 1,'type' => 'varchar'),
// 					'frozen' => array('mandatory' => 0,'type' => 'varchar'),
// 					'discontinued' => array('mandatory' => 0,'type' => 'varchar'),
// 					'i_ppayment' => array('mandatory' => 0,'type' => 'integer'),
// 					'action' => array('mandatory' => 1,'type' => 'varchar')
// 				),
// 				'voice_calls' => array(
// 					'general' => array(
// 						'legal_intercept' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'call_recording' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'auto_record_outgoing' => array('mandatory' => 3,'type' => 'integer','html' => 'select','dependence' => array('attribute' => 'call_recording','value_in' => array('Y'))),
// 						'auto_record_incoming' => array('mandatory' => 3,'type' => 'integer','html' => 'select','dependence' => array('attribute' => 'call_recording','value_in' => array('Y'))),
// 						'auto_record_redirected' => array('mandatory' => 3,'type' => 'integer','html' => 'select','dependence' => array('attribute' => 'call_recording','value_in' => array('Y'))),
// 						'rtpp_level' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'cli_trust_accept' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'cli_trust_supply' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'i_moh' => array('mandatory' => 0,'type' => 'varchar','html' => 'select')
// 					),
// 					'Customer' => array(
// 						'call_parking' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'park_prefix' => array('mandatory' => 2,'type' => 'varchar', 'dependence' => array('attribute' => 'call_parking','value_in' => array('Y'))),
// 						'release_prefix' => array('mandatory' => 2,'type' => 'varchar','dependence' => array('attribute' => 'call_parking','value_in' => array('Y'))),
// 						'first_login_greeting' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox')
// 					),
// 					'Account' => array(
// 						'redirect_number' => array('mandatory' => 0,'type' => 'varchar'),
// 						'voice_service_policy' => array('mandatory' => 0,'type' => 'varchar','html' => 'select')
// 					)
// 				),
// 				'incoming_calls' => array(
// 					'general' => array(
// 						'distinctive_ring_vpn' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'endpoint_redirect' => array('mandatory' => 0,'type' => 'varchar','html' => 'select')
// 					),
// 					'Customer' => array(
// 						'group_pickup' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'group_pickup_prefix' => array('mandatory' => 2,'type' => 'varchar','dependence' => array('attribute' => 'group_pickup','value_in' => array('Y')))
// 					),
// 					'Account' => array(
// 						'um_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'follow_me_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'max_forwards' => array('mandatory' => 0,'type' => 'integer'),
// 						'timeout' => array('mandatory' => 0,'type' => 'integer'),
// 						'call_processing' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'default_action' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'clip' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'call_wait_limit' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox')
// 					)
// 				),
// 				'outgoing_calls' => array(
// 					'general' => array(
// 						'cli' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'cli_batch' => array('mandatory' => 3,'type' => 'varchar','html' => 'select','dependence' => array('attribute' => 'cli','value_not_in' => array('N','^'))),
// 						'display_number_check' => array('mandatory' => 3,'type' => 'varchar','html' => 'select','dependence' => array('attribute' => 'cli','value_not_in' => array('N','^'))),
// 						'display_name_override' => array('mandatory' => 3,'type' => 'varchar','html' => 'select','dependence' => array('attribute' => 'cli','value_not_in' => array('N','^'))),
// 						'centrex' => array('mandatory' => 3,'type' => 'varchar','dependence' => array('attribute' => 'cli','value_not_in' => array('N','^'))),
// 						'clir' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'clir_hide' => array('mandatory' => 2,'type' => 'varchar','dependence' => array('attribute' => 'clir','value_in' => array('Y'))),
// 						'clir_show' => array('mandatory' => 2,'type' => 'varchar','dependence' => array('attribute' => 'clir','value_in' => array('Y')))
// 					),
// 					'Customer' => array(
// 						'paging' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'paging_prefix' => array('mandatory' => 2,'type' => 'varchar','dependence' => array('attribute' => 'paging','value_in' => array('Y')))
// 					),
// 					'Account' => array(
// 						'display_name' => array('mandatory' => 3,'type' => 'varchar','dependence' => array('attribute' => 'display_name_override','value_in' => array('N'))),
// 						'display_number' => array('mandatory' => 3,'type' => 'varchar','dependence' => array('attribute' => 'display_name_override','value_in' => array('N'))),
// 						'iso_639_1' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'favourite_numbers' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'emergency' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'call_barring' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'i_routing_plan' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'phonebook_enabled' => array('mandatory' => 0,'type' => 'varchar','html' => 'checkbox'),
// 						'abbrev_mode' => array('mandatory' => 3,'type' => 'integer','html' => 'select','dependence' => array('attribute' => 'phonebook_enabled','value_in' => array('Y'))),
// 						'max_favorites' => array('mandatory' => 3,'type' => 'integer','dependence' => array('attribute' => 'phonebook_enabled','value_in' => array('Y'))),
// 						'favorite_allowed_patterns' => array('mandatory' => 3,'type' => 'varchar','dependence' => array('attribute' => 'phonebook_enabled','value_in' => array('Y'))),
// 						'favorite_change_lock_days' => array('mandatory' => 3,'type' => 'double','html' => 'select','dependence' => array('attribute' => 'phonebook_enabled','value_in' => array('Y'))),
// 						'voice_pass_through' => array('mandatory' => 0,'type' => 'integer','html' => 'select'),
// 						'outgoing_access_number' => array('mandatory' => 2,'type' => 'varchar','html' => 'select','dependence' => array('attribute' => 'voice_pass_through','value_in' => array('Y')))
// 					)
// 				),
...
// 					'Customer' => array(),
// 					'Account' => array(
// 						'voice_location' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'primary_location' => array('mandatory' => 0,'type' => 'varchar'),
// 						'allow_roaming' => array('mandatory' => 0,'type' => 'varchar','html' => 'select'),
// 						'voice_authentication' => array('mandatory' => 0,'type' => 'varchar')
// 					)
// 				)
// 			);
// 		}

// 		return $realm && isset($fields[$form]) && isset($fields[$form]['general']) && isset($fields[$form][$realm]) ?
// 				array_merge($fields[$form]['general'],$fields[$form][$realm]) :
// 					(isset($fields[$form]) ? $fields[$form] : array());
// 	}

	private static function _GetFieldClass($field_options)
	{
		$class = '';
		...
		if($field_options)
		{
			...
			if('string' == $field_options['type'])
			{
				$class .= ' check letters-digits-dashes-spaces'.(isset($field_options['html']) && 'textarea' == $field_options['html'] ? "-punctuation" : "");
			}
			if('text' == $field_options['type'])
			{
				$class .= ' check letters-digits-dashes-spaces-punctuation';
			}
			elseif ('integer' == $field_options['type'])
			{
				$class .= ' check digits';
			}
			elseif ('double' == $field_options['type'])
			{
				$class .= ' check float';
			}
			elseif('email' == $field_options['type'])
			{
				$class .= ' check email';
			}
		}
	...
	public static function GetLocations($key="states")
	{
		static $output;
		...
		if(is_null($output))
		{
			...
			foreach($output["states"] as $iso_3166_1_a2 => $data)
			{
				$output["countries"][$iso_3166_1_a2] = $data["country"];
			}
		}

		return isset($output[$key]) ? $output[$key] : NULL;
	}
}
